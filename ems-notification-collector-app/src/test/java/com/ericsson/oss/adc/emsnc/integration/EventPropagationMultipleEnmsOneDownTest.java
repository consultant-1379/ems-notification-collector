...
@TestPropertySource(
    properties = {
      "emsnc.kafka.emsnc-internal.topic-count="
          + EventPropagationMultipleEnmsOneDownTest.TEST_TOPIC_COUNT,
...
public class EventPropagationMultipleEnmsOneDownTest {
  @TestConfiguration
  ...
  @SneakyThrows
  @Test
  public void testBasicEventPropagationWith2EnmsOneGoingDownAndComingBack() {
    ...
    simulatedEnm2.getWireMockServer().resetMappings();
    simulatedEnm1.setMaxEventCycles(2);
    simulatedEnm2.setMaxEventCycles(2);
    await()
        .atMost(60, TimeUnit.SECONDS)
        .until(() -> dmaapKafkaCollector.getCollectedMessages().size() == 300);

    simulatedEnm1.setMaxEventCycles(3);
    simulatedEnm2.setMaxEventCycles(3);
    simulatedEnm2.addDefaultStubs();
    await()
        .atMost(60, TimeUnit.SECONDS)
        .until(() -> dmaapKafkaCollector.getCollectedMessages().size() == 600);

    assertEquals(600, dmaapKafkaCollector.getCollectedMessages().size());
  }
}
