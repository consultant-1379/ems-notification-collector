...
public class ConnectedSystemsStateTest {
  ...
  public static class SimulationConfiguration {
    ...
    public SimulatedEnm simulatedEnm() {
      ...
      targets.put("RadioNode", 10);
      targets.put("Router6675", 10);
      return SimulatedEnm.builder()
          .targets(targets)
          .maxEventCycles(1)
          .eventsPerCycle(1)
          .build()
  ...
  @SneakyThrows
  @Test
  public void testEventPropagationContinuesWithoutConnectedSystems() {
    await()
        .atMost(60, TimeUnit.SECONDS)
        .until(() -> dmaapKafkaCollector.getCollectedMessages().size() == 20);

    assertFalse(dmaapKafkaCollector.getCollectedMessages().isEmpty());
    simulatedConnectedSystems.shutdown();
    simulatedEnm.setMaxEventCycles(2);
    await()
        .atMost(60, TimeUnit.SECONDS)
        .until(() -> dmaapKafkaCollector.getCollectedMessages().size() == 40);
  }
}
